return {
  ...
  config = function(_, opts)
    ...
    -- Disable go to definition phpactor
    local function disable_phpactor_definitions(client)
    ...
    local servers = {
      phpactor = {
        ...
        on_attach = disable_phpactor_definitions,
      },
      intelephense = {
        capabilities = capabilities,
        settings = {
      ...
      pyright = {
        ...
        settings = {
          python = {
            analysis = {
              pythonPath = os.getenv("VIRTUAL_ENV") and os.getenv("VIRTUAL_ENV") .. "/bin/python3"
    ...
    for server, config in pairs(servers) do
      if server == "ts_ls" and is_vue_project() then
        print("Skipping tsserver")
      elseif server == "volar" and not is_vue_project() then
        print("Skipping volar")
      else
