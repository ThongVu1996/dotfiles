-- File: lua/config/plugins/outline-sidebar.lua
return {
	...
	config = function()
				...
				if is_valid_buffer(State.source_buf) then
					...
					if target_line <= line_count then
						vim.api.nvim_win_set_cursor(
							State.source_win,
							{ target_line, symbol_data.range.start.character }
						)
						vim.cmd("norm! zz")
					end
				end
		...
		function render_outline_from_cache()
		...
		_G.ToggleOutline = function()
			...
			-- OPEN
			-- [NEW] Tắt Explorer trước khi mở Outline
			close_explorer()
		...
		-- [NEW] Tự động đóng Outline nếu người dùng mở Explorer
		vim.api.nvim_create_autocmd("FileType", {
			group = grp,
			pattern = CONFIG.explorer_filetypes,
			callback = function()
				-- Nếu Outline đang mở mà gặp Explorer bật lên -> Đóng Outline
				if State.win_id and vim.api.nvim_win_is_valid(State.win_id) then
					close_outline_full()
				end
			end,
